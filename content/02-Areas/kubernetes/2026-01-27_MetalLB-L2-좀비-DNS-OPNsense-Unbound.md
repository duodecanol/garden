...
```
dig gateway.imagegen-gpu.intranet.example.internal
;; ANSWER SECTION:
gateway.imagegen-gpu.intranet.example.internal. 3600 IN A 10.0.0.2
...
- **MetalLB (L2Advertisement)**: "이 IP(`10.0.0.2`)를 가진 기기가 누구인가?"라는 ARP 요청에 "나(내 MAC)야!"라고 손드는 행위.
...
```bash
arp -an | grep 10.0.0.2
ip neigh show | grep 10.0.0.2
```
...
```bash
dig @<내부_DNS_IP> gateway.imagegen-gpu.intranet.example.internal
```
...
2. `dig` 질의 → `systemd-resolved`는 캐시가 비었으므로 상위 DNS(내부 DNS, 8.8.8.8 등)에 물음.
3. 상위 DNS가 아직 레코드(`10.0.0.2`)를 가지고 있으므로 응답.
4. `systemd-resolved`가 응답을 받자마자 **다시 캐싱**하고 보여줌.

### 확인: 로컬 캐시를 거치지 않고 상위 DNS에 직접
```bash
resolvectl status | grep "DNS Server"     # 상위 DNS 주소 확인
dig @<상위_DNS_IP> gateway.imagegen-gpu.intranet.example.internal
```
여기서도 응답이 오면 **내 PC가 아니라 내부/외부 DNS 서버에 레코드가 안 지워지고 남은 것**(ExternalDNS 등이 삭제 처리 못 하고 죽었을 확률 99%).

### 그 외 가능성
- **`/etc/hosts` 정적 매핑**: DNS를 안 거치고 항상 응답. `grep "gateway.imagegen-gpu" /etc/hosts` → 있으면 삭제.
...
## 4. 범인 확정: 상위 DNS = OPNsense (`10.0.0.53`)
...
```
→ 범인은 **`10.0.0.53`(상위 DNS = OPNsense 게이트웨이)**. 로컬 캐시를 아무리 비워도 `dig`를 치는 순간 이 서버에 질의가 가고, 이 서버가 여전히 "그 도메인은 `10.0.0.2`"라 응답하기 때문.
```bash
dig @10.0.0.53 gateway.imagegen-gpu.intranet.example.internal    # 즉답 → 이 서버가 범인
```
...
`10.0.0.53`이 OPNsense면, DNS 처리 방식별로:

### 방법 1 — BIND 플러그인 (ExternalDNS RFC2136 연동 시 유력)
1. **Services → BIND → Configuration → Zones**
2. 해당 도메인(예: `intranet.example.internal`) Zone 편집
...
2. Hostname `gateway`, Domain `imagegen-gpu...` 항목 찾기
3. 휴지통 → **Apply**
...
dig @10.0.0.53 gateway.imagegen-gpu.intranet.example.internal
...
> 같은 dangling 레코드의 **AWS/Cloudflare 버전**: [[2026-03-26_ArgoCD-Replace-ELB-재생성-ExternalDNS]] · OPNsense 라우터 운영: [[02-Areas/vivident-office-network/index|office-network]] · 이 클러스터의 MTU 트러블슈팅: [[2026-01-26_K3s-MetalLB-Tailscale-MTU-Flannel-트러블슈팅]]
